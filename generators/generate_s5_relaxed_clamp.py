#!/usr/bin/env python3
"""
S5: Relaxed clamping — raise threshold from 150% to 200%, and exempt cells
with n>=5 (they have enough data to be trusted).
Baseline clamps anything >150% of neighbor median for cells with n<10.
This is too aggressive for legitimately expensive micro-neighborhoods
(Noho, Hudson Square) where a $7k cell surrounded by $5k cells gets
flattened to $5k.
"""
...
# Spatial smoothing (same as baseline — 0.008 fixed radius)
SMOOTH_RADIUS = 0.008
smoothed_points = []
...
print(f"Spatial smoothing applied")

# ─── KEY CHANGE: Relaxed neighbor median clamping ─────────────────────
# Baseline: threshold=1.50, exempt n>=10
# S5:       threshold=2.00, exempt n>=5
# This lets legitimately expensive pockets (Noho $7k surrounded by $5k EV)
# survive. Only truly extreme outliers (>200% of neighbors) get clamped.
CLAMP_RADIUS = 0.015
CLAMP_THRESHOLD = 2.00      # was 1.50 — now only clamp if >200% of neighbor median
CLAMP_MAX_N = 5             # was 10 — cells with n>=5 are trusted, never clamped
clamped_count = 0
for i, hp in enumerate(heat_points):
    ...
    if len(neighbors) >= 1:
        ...
        if hp["rent"] > neighbor_med * CLAMP_THRESHOLD:
            old = hp["rent"]
            hp["rent"] = int(neighbor_med * CLAMP_THRESHOLD)  # soft clamp to 200% instead of hard clamp to median
            clamped_count += 1
            print(f"  CLAMPED: ({hp['lat']},{hp['lng']}) ${old:,} → ${hp['rent']:,} (n={hp['count']})")
print(f"Relaxed clamping: {clamped_count} points (n<{CLAMP_MAX_N}, >{CLAMP_THRESHOLD:.0%} of neighbor median)")

heat_points.sort(key=lambda x: (-x["rent"], x["lat"]))
print(f"Heat points: {len(heat_points)} (dropped {dropped_thin} thin cells)")

# Write output
with open("/tmp/heat_points_s5_relaxed_clamp.js", "w") as f:
...
print(f"\nOutput: /tmp/heat_points_s5_relaxed_clamp.js")
