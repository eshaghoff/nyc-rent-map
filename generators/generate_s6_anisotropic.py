#!/usr/bin/env python3
"""
S6: Anisotropic smoothing — smoothing weight falls off with rent difference,
not just distance. Neighbors with similar rents pull strongly; neighbors
with very different rents barely contribute. This preserves sharp rent
boundaries (Noho↔Chinatown, DUMBO↔Vinegar Hill) while still smoothing
gradual transitions (UES↔Yorkville, Park Slope↔Prospect Heights).

Formula: w = (1/dist) * exp(-rent_diff² / sigma²)
sigma controls how much rent difference is tolerated before cutting off.
"""
...
# ─── Anisotropic spatial smoothing ──────────────────────────────────────
# Key idea: weight = (1/dist) * exp(-rent_diff² / sigma²)
# sigma = how much rent difference (in $) before a neighbor's influence
# drops to ~37% (1/e). At 2*sigma it's ~14%, at 3*sigma it's ~5%.
# sigma=1000 means a $1000 rent gap cuts influence to 37%.
# A $3000 gap (Noho $6k vs Chinatown $3k) → exp(-9) ≈ 0.01% — basically zero.
SMOOTH_RADIUS = 0.008
RENT_SIGMA = 1000  # dollars — controls sharpness of rent boundaries
RENT_SIGMA_SQ = RENT_SIGMA ** 2

smoothed_points = []
aniso_dampened = 0  # count of cells where anisotropic reduced smoothing significantly
for i, hp in enumerate(heat_points):
    self_weight = 2.0
    total_weight = self_weight
    weighted_rent = hp["rent"] * self_weight
    isotropic_rent = hp["rent"] * self_weight  # track what baseline smoothing would give
    isotropic_weight = self_weight
    for j, other in enumerate(heat_points):
        ...
        if dist < SMOOTH_RADIUS and dist > 0:
            dist_w = 1.0 / dist
            # Isotropic (baseline equivalent)
            isotropic_weight += dist_w
            isotropic_rent += other["rent"] * dist_w
            # Anisotropic: penalize neighbors with very different rents
    ...
    smoothed = int(round(weighted_rent / total_weight))
    iso_smoothed = int(round(isotropic_rent / isotropic_weight))
    if abs(smoothed - iso_smoothed) > 100:
        aniso_dampened += 1
...
print(f"  {aniso_dampened} cells differ >$100 from isotropic baseline")
...
# Noho / Nolita / Hudson Square diagnostic
